...
module Forum
  class Server < Sinatra::Base
    ...
    get '/topic_thread/:id' do
      ...
      @post_list = @@db.exec_params("SELECT posts.*, users.name, users.img_link, users.email FROM posts, users WHERE posts.topics_id = #{@this_topic} AND user_id = users.id ORDER BY posts.created_at")
      erb :topic_thread
    end
